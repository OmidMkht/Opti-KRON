...
carries, so a two-phase slack gets two values and a three-phase slack three.

Angles follow the *phase*, not the position -- a slack on phases `a` and `c` is
...
function powerflow(net::Network;
    ...
    for iteration in 1:maxiter
        ...
        # Once the step stops shrinking, the iteration has hit the accuracy
        # floor of the linear solve and more passes buy nothing. Bail out and
        # say so, rather than spinning to `maxiter` and blaming the feeder.
